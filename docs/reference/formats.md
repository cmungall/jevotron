...
Give jevotron a file and it chooses a parser from the extension. Each parser
...
Extensions are case-insensitive. Every listed format also accepts gzip,
...
For XML, RDF, spreadsheets, Parquet, database exports, or specialized chunking,
...
in Python; combining it with `--format` or `--format-option` is an error.
